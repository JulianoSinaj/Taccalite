...
- Almost every photograph in `public/images/` is a real shot of the bottega, carried over from the shop's previous site: banco-carni-macinati, banco-carni-bovino, banco-carni-vetrina, salumi-appesi-stagionatura, gastronomia-teglie-forno, spiedini-verdure-banco, lonza-suino-brado. Anything without one uses a labelled placeholder rather than stock
- `pasta-artigianale-bottega` (a Strampelli packet beside Paolo's business card) was in that list and is deleted, along with `tagliere-formaggi` — both were in `public/` with nothing rendering them, in the source or in the database. `git show a041c70^:public/images/pasta-artigianale-bottega.jpg` gets the shop's own one back if a pantry or Marche section ever wants it
...
- `marmo-banco-texture` (Pexels 4705833, Pexels License, no attribution required) is the one stock file that is **not** a stand-in: it is travertine, used at 55% under the home hero's plate as the counter surface. A surface is the only thing stock can safely be on this site — every stock *interior* worth using turned out to be a named competitor's shopfront (Simoni, Zuarina, Pio Tosini) or a watermarked press photo, which is a stranger's shop standing under "dal 1946". If it is ever replaced by the shop's own marble, nothing else has to change
- `prosciutto-crudo-tagliere` (Pexels 13728911, Pexels License, no attribution required) is the home hero, chosen by the owner, and a stand-in until the shop photographs its own crudo. The test a stock *subject* has to pass here is that it identifies nobody else: no shopfront, no signage, no producer's label, no face. Two other frames from the same shoot fail it — "Roquefort" is legible on the foil — so use this id and not its neighbours
